...
public class EHSystem {
	...
	public EHSystem(String configFile, int node){
	    ...
	    try {
			...
			System.err.println ("Energy Harvesting System Configuration file " + configFile + " could not be read.. Exiting...");
			System.exit(-1);
		} catch (IOException e) {
			System.err.println ("Energy Harvesting System Configuration file " + configFile + " could not be loaded.. Exiting...");
			System.exit(-1);
		}
	    ...
	    if (config.getProperty("source.type").equalsIgnoreCase("photovoltaic")){
	    	source = new PhotovoltaicCell(config.getProperty("source.name"), config.getProperty("source.outputpower.lookuptable"));
	 ...
	 // Initializing environment for energy source
	    ...
	    chargeInterval= Double.parseDouble(config.getProperty("source.environment.sampleinterval")); //in seconds, defines how frequently charge should be updated 
	    
	    // Initializing Harvester
	    harvester = new Harvester(config.getProperty("harvester.name"), 
	...
	public void harvestCharge(){
		...
		// TODO: handle the out of range values of outputpower. if envValue is too large, we should get maximum output power 
		...
		double energy = source.getOutputEnergy(envValue, chargeInterval)*harvEfficiency/1000; // micro joules /1000 = millijoules
	...
	public void consumeCharge (double energyConsumed){
		...
		//System.out.println("energy depleted:\t" + energyConsumed ); 
	}
	...
	public static void main(String[] args) {
		EHSystem ehSys = new EHSystem ("/home/raza/raza@murphysvn/code/java/eclipseIndigo/Senseh/EH.config", 1);
		for (int i = 0; i < 1; i++){
			ehSys.harvestCharge();
		}
	}*/
